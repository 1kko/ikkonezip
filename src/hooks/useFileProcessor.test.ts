...
describe('useFileProcessor', () => {
  ...
  describe('renameFolder', () => {
    it('updates normalizedPath of every descendant file', async () => {
      const { result } = renderHook(() => useFileProcessor());

      await act(async () => {
        await result.current.addFiles([
          createMockFileWithPath('a.txt', 'root/a.txt'),
          createMockFileWithPath('b.txt', 'root/sub/b.txt'),
          createMockFileWithPath('c.txt', 'other/c.txt'),
        ]);
      });

      act(() => {
        result.current.renameFolder('root', 'renamed');
      });

      const paths = result.current.files.map((f) => f.normalizedPath).sort();
      expect(paths).toEqual(['other/c.txt', 'renamed/a.txt', 'renamed/sub/b.txt']);
    });

    it('renames a nested folder, preserving the parent prefix', async () => {
      const { result } = renderHook(() => useFileProcessor());

      await act(async () => {
        await result.current.addFiles([
          createMockFileWithPath('a.txt', 'root/sub/a.txt'),
          createMockFileWithPath('b.txt', 'root/sub/deeper/b.txt'),
          createMockFileWithPath('c.txt', 'root/other.txt'),
        ]);
      });

      act(() => {
        result.current.renameFolder('root/sub', 'renamed');
      });
      ...
      expect(paths).toEqual([
        'root/other.txt',
        'root/renamed/a.txt',
        'root/renamed/deeper/b.txt',
      ]);
    });

    it('strips slashes from the new folder name', async () => {
      const { result } = renderHook(() => useFileProcessor());
      ...
      act(() => {
        result.current.renameFolder('root', '../evil/name');
      });

      expect(result.current.files[0].normalizedPath).toBe('..evilname/a.txt');
    });

    it('is a no-op for empty name', async () => {
      const { result } = renderHook(() => useFileProcessor());
      ...
      act(() => {
        result.current.renameFolder('root', '   ');
      });
    ...
    it('does not touch files outside the folder', async () => {
      ...
      await act(async () => {
        await result.current.addFiles([
          createMockFileWithPath('a.txt', 'root/a.txt'),
          createMockFileWithPath('b.txt', 'rootOther/b.txt'),
        ]);
      });
      ...
      const paths = result.current.files.map((f) => f.normalizedPath).sort();
      expect(paths).toEqual(['renamed/a.txt', 'rootOther/b.txt']);
    });
  });

  describe('removeFolderByPath', () => {
    it('removes every descendant of the folder', async () => {
      const { result } = renderHook(() => useFileProcessor());
      ...
      act(() => {
        result.current.removeFolderByPath('root');
      });

      const paths = result.current.files.map((f) => f.normalizedPath);
      expect(paths).toEqual(['other/c.txt']);
    });

    it('does not remove files whose path only shares a prefix string', async () => {
      const { result } = renderHook(() => useFileProcessor());
      ...
      const paths = result.current.files.map((f) => f.normalizedPath);
      expect(paths).toEqual(['rootOther/b.txt']);
    });
    ...
    it('is a no-op for an empty path', async () => {
      ...
      act(() => {
        result.current.removeFolderByPath('');
      });
